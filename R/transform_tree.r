follow_conjunctions <- function(tokens, conj) {
...
unpack_conjunctions <- function(tokens, bypass, link_children, only_first_level=T, copy=T) {
  parent = NULL; relation = NULL; .CONJ_LEVEL = NULL; .MATCH_ID = NULL; .ADDED = NULL
  
  conj = follow_conjunctions(tokens, bypass)
  if (nrow(conj) > 0) {
    ...
    if (!is.null(link_children)) {
      ...
      if (!is.null(add)) {
        ...
        fam_match = match(fam$.MATCH_ID, add$token_id)
        fam_uberparent = add$parent[fam_match]
        add$token_id = (add$parent) + token_sub_id(add$token_id, largest_id)
        
        if (nrow(fam) > 0) {
          fam[, .CONJ_LEVEL := add$.CONJ_LEVEL[fam_match]]
