...
#' @param udpipe_model_path If udpipe_model is used, this path wil be used to look for the model, and if the model doesn't yet exist it will be downloaded to this location. If no path is given, the directory in which the \link{corpustools} package was installed will be used. 
#'
#' @return
#' @export
#'
...
udpipe_tokenindex <- function(text, udpipe_model='english', udpipe_model_path=getOption("corpustools_resources", NULL)) {
  tc = create_tcorpus(text, udpipe_model=udpipe_model, use_parser=T)
